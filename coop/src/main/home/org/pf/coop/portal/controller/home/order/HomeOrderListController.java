...
public class HomeOrderListController extends HomeBaseController {
	...
	public String listOrder(@ModelAttribute Order order, Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			request.getSession().setAttribute("homeSearch_order", order);
	...
	public String listOrder(Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			Order obj = (Order) request.getSession().getAttribute("homeSearch_order");
			...
			request.getSession().setAttribute("homeSearch_order", obj);
			...
			request.getSession().setAttribute("listOrderHome_pageNumber", pageNumber);
			request.getSession().setAttribute("listOrderHome_totalPages", totalPages);
	...
	public String listOrder(@PathVariable String whichPage, Model model, Principal principal, HttpServletRequest request) {
		...
		try {
			int pageNumber = (int) request.getSession().getAttribute("listOrderHome_pageNumber");
			int totalPages = (int) request.getSession().getAttribute("listOrderHome_totalPages");
			
			if ("previous".equals(whichPage)) {
				if (pageNumber == 0) return "redirect:/home/order/list";
				else {
					pageNumber--; 
				}
