package org.pf.coop.portal.controller.mobile.open.gallery;
...
@Controller
@RequestMapping("/mobile/open/gallery")
public class MobileOpenGalleryListController extends MobileBaseController {
	...
	public String listGallery(@ModelAttribute Gallery gallery, Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			request.getSession().setAttribute("mobileOpenSearch_gallery", gallery);
				
			return "redirect:/mobile/open/gallery/list";
		} catch (Exception e) {
	...
	public String listGallery(Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			Gallery obj = (Gallery) request.getSession().getAttribute("mobileOpenSearch_gallery");
			...
			request.getSession().setAttribute("mobileOpenSearch_gallery", obj);
			...
			request.getSession().setAttribute("listMobileOpenGalleryModerator_pageNumber", pageNumber);
			request.getSession().setAttribute("listMobileOpenGalleryModerator_totalPages", totalPages);
			
			return "mobile/open/gallery/list";
	...
	public String listGallery(@PathVariable String whichPage, Model model, Principal principal, HttpServletRequest request) {
		...
		try {
			int pageNumber = (int) request.getSession().getAttribute("listMobileOpenGalleryModerator_pageNumber");
			int totalPages = (int) request.getSession().getAttribute("listMobileOpenGalleryModerator_totalPages");
			
			if ("previous".equals(whichPage)) {
				if (pageNumber == 0) return "redirect:/mobile/open/gallery/list";
				else {
					pageNumber--; 
				}
