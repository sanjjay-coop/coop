...
public class OpenDocumentListController extends BaseController {
	...
	public String listDocument(@ModelAttribute Document document, Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			request.getSession().setAttribute("openSearch_document", document);
				
			return "redirect:/open/document/list";
		} catch (Exception e) {
	...
	public String listDocument(Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			Document obj = (Document) request.getSession().getAttribute("openSearch_document");
			...
			request.getSession().setAttribute("openSearch_document", obj);
	...
	public String listDocument(@PathVariable String whichPage, Model model, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			if ("previous".equals(whichPage)) {
				if (pageNumber == 0) return "redirect:/open/document/list";
				else {
					pageNumber--; 
				}
